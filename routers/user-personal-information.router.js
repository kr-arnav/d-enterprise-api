...
router
    .get('/:mobile', (req, res) => {
        UserInformation.
            ...
            .then(d => res.status(200).json({ message: 'Personal-Information retrieved', pInfo: d.personalInformation }))
            .catch(d => res.status(400).json({ message: 'Something went wrong', data: d }))
    })
    .post('/:mobile', UserAuthService, (req, res) => {
        UserInformation
            .updateOne({ mobile: req.params.mobile }, { $set: { personalInformation: req.body } })
            .then(d => {res.status(200).json({ message: 'Personal-Information Updated', data: d })})
