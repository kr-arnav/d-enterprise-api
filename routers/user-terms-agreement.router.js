...
router
    .get('/:mobile', (req, res) => {
        UserInformation.
            ...
            .then(d => res.status(200).json({ message: 'Terms-Agreement retrieved', aTerms: d.termsAgreed }))
            .catch(d => res.status(400).json({ message: 'Something went wrong', data: d }))
    })
    .post('/:mobile', UserAuthService, (req, res) => {
        UserInformation
            .updateOne({ mobile: req.params.mobile }, { $set: { termsAgreed: req.body } })
            .then(d => res.status(200).json({ message: 'Terms-Agreement Updated', data: d }))
